...
export default async function CharactersPage() {
  ...
  const favorite = profile?.favorite_character ?? null;
    ...
    <main className="mx-auto max-w-5xl px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-slate-300">
          ...
          character stays fully in their own voice. Star your favorite to jump straight into chat
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {characters.map((character) => (
          <div
            key={character.id}
            className="group relative flex flex-col rounded-[2rem] border border-white/10 bg-white/[0.06] shadow-xl backdrop-blur transition-all hover:border-brand-500/50 hover:bg-white/[0.1]"
          >
            <FavoriteButton characterId={character.id} isFavorite={favorite === character.id} />

            {!character.enabled && (
              <span
                className="absolute right-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-yellow-400/20 text-yellow-300"
